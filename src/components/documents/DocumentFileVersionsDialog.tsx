...
import { DocumentFile } from '@/hooks/useDocumentFiles';

interface DocumentFileVersionsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  file: DocumentFile | null;
  getVersions: (fileId: string) => Promise<DocumentFile[]>;
  onDownload: (filePath: string, fileName: string) => void;
  onRestore: (versionId: string) => void;
  isAdmin: boolean;
}

export const DocumentFileVersionsDialog = ({
  open,
  onOpenChange,
  file,
  getVersions,
  ...
  onRestore,
  isAdmin,
}: DocumentFileVersionsDialogProps) => {
  const [versions, setVersions] = useState<DocumentFile[]>([]);
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Verzióelőzmények: {file?.file_name}
          </DialogTitle>
          <DialogDescription>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => onDownload(version.file_path, `${version.file_name}_v${version.version}`)}
                        title="Letöltés"
                      >
